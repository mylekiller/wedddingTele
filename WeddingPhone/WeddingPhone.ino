...
Bounce receiverButton = Bounce(0, 8);
...
void loop() {
  ...
  receiverButton.update();
  //Step 0 - In order to do anything the receiver needs to be picked up
  if (receiverButton.risingEdge()) {
    //Ensure last round is cleaned up and done
    //Play Intro
    //Start Recording
  }

  if (receiverButton.read() == 1) {
    //Check Mode Continue doing what mode says to do
  }

  if (receiverButton.fallingEdge()) {
    //End Playback/Recording as necessary and clean up
  }
...
void stopPlayingIntro() {
  ...
  if (mode == playbackActive) playWav1.stop();
  mode = notRecording;
}
